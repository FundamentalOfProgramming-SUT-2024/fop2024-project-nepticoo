...
int get_all_users_scoreboard(USER_S* all_users) {
    ...
    while((ent = readdir(dir))!= NULL) {
        ...
        sprintf(filepath, "./users/%s/user_s.dat", ent->d_name);
        FILE *file = fopen(filepath, "rb");
        fread(&tmp_users[n++], sizeof(USER_S), 1, file);
        fclose(file);
...
int scoreboard_menu() {
    ...
    while(1) {
        ...
        attroff(COLOR_PAIR(106));
        attron(A_BOLD | A_BLINK | COLOR_PAIR(108));
        mvprintw(7, (COLS - 16) / 2, "THE TOP PLAYERS!");
        attroff(A_BOLD | A_BLINK | COLOR_PAIR(108));
        attron(A_DIM);
        attron(A_UNDERLINE);
        mvprintw(33, (COLS - 12) / 2, "(page %d/%d)", selected + 1, pages);
        attroff(A_UNDERLINE);
        mvprintw(8, (COLS - 35) / 2, "(Scroll the list using UP and DOWN)");
        mvprintw(9, (COLS - 29) / 2 ,"(press any other key to EXIT)");
        attroff(A_DIM);
        ...
        for(int i = selected * 10; i < (selected + 1) * 10; i++) {
            if(i >= users_count)
                break;
            if(!strcmp(user.username, all_users[i].username)) {
                attron(A_BOLD | A_UNDERLINE);
                for(int j = 0; j < lenght; j++) {
                    mvprintw(13 + (2*i) - (selected*20), ((COLS - lenght) / 2) + j, " ");
                }
                attroff(A_BOLD | A_UNDERLINE);
                mvprintw(13 + (2*i) - (selected*20), ((COLS - lenght) / 2) + start_from[0], "~>");
            }
            if(!strcmp(user.username, all_users[i].username)) {
                attron(A_UNDERLINE);
            }
            if(i == 0) {
                mvprintw(13 + (2*i) - (selected*20), ((COLS - lenght) / 2) + start_from[1], "\xF0\x9F\xA5\x87");
                attron(COLOR_PAIR(109) | A_ITALIC);
                attron(A_DIM);
                mvprintw(13 + (2*i) - (selected*20), ((COLS - lenght) / 2) + start_from[3] + strlen(all_users[i].username), "(GOAT)");
                attroff(A_DIM);
            }
            if(i == 1) {
                mvprintw(13 + (2*i) - (selected*20), ((COLS - lenght) / 2) + start_from[1], "\xF0\x9F\xA5\x88");
                attron(COLOR_PAIR(110) | A_ITALIC);
                attron(A_DIM);
                mvprintw(13 + (2*i) - (selected*20), ((COLS - lenght) / 2) + start_from[3] + strlen(all_users[i].username), "(Master)");
                attroff(A_DIM);
            }
            if(i == 2) {
                mvprintw(13 + (2*i) - (selected*20), ((COLS - lenght) / 2) + start_from[1], "\xF0\x9F\xA5\x89");
                attron(COLOR_PAIR(111) | A_ITALIC);
                attron(A_DIM);
                mvprintw(13 + (2*i) - (selected*20), ((COLS - lenght) / 2) + start_from[3] + strlen(all_users[i].username), "(Warrior)");
                attroff(A_DIM);
            }
            // if(i > 2) {
            //     attron(A_DIM);
            ...
            mvprintw(13 + (2*i) - (selected*20), ((COLS - lenght) / 2) + start_from[2], "%d", i+1);
            mvprintw(13 + (2*i) - (selected*20), ((COLS - lenght) / 2) + start_from[3], "%s", all_users[i].username);
            mvprintw(13 + (2*i) - (selected*20), ((COLS - lenght) / 2) + start_from[4], "%d", all_users[i].points);
            mvprintw(13 + (2*i) - (selected*20), ((COLS - lenght) / 2) + start_from[5], "%d", all_users[i].all_saved_golds);
            mvprintw(13 + (2*i) - (selected*20), ((COLS - lenght) / 2) + start_from[6], "%d", all_users[i].finished_games);
            time_t now = time(NULL);
            double diffrence_sec = difftime(now, all_users[i].first_game_time);
            double diffrence_hrs = diffrence_sec / 3600;
            mvprintw(13 + (2*i) - (selected*20), ((COLS - lenght) / 2) + start_from[7], "%.2lf", diffrence_hrs);
            if(!strcmp(user.username, all_users[i].username)) {
                attroff(A_UNDERLINE);
            }
            if(i == 0) {
